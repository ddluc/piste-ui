...
const SortIcon = styled.div<{ isActiveSort: boolean, sortDirection: string }>`
  ...
  ${({ isActiveSort, sortDirection }) => {
    if (isActiveSort && sortDirection === 'desc') {
      return css`animation: 200ms ease-in 0ms forwards ${animations.flipDown};`;
    }
    return css`animation: 200ms ease-in 0ms forwards ${animations.flipUp};`;
  }};
  transform-origin: center; // Ensure rotation happens around the center
  transition: transform 0.3s ease;
  ${({ theme, isActiveSort }) => (isActiveSort ? '' : `color: ${theme.palette.accent.shades[1]}`)};
`;
